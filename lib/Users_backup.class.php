...
Class Users{
	...
	*********************************/
	...
	private $host_db = 'localhost:3307';
	private $user_db = 'root';
	private $pass_db = '';
	private $name_db = 'webapp';
	// gli url che gestinranno le operazioni di login
	public $Urls = array(
						'login_page' 	=> 'http://192.168.1.5/app/index.php',
						'register_page'	=> 'http://192.168.1.5/app/registrati.php',
						'logout_page'	=> 'http://192.168.1.5/app/logout.php',
						);
	...
	public function get_register_form(){
	  ...
	  $minuti = mysql_result($result,0);
		...
		$html = '
          ...
          <div class="page-header">
              ...
              <p><input type="text" name="indirizzo" id="reg_indirizzo" placeholder="Città e Via" class="material" required/><br>
                <input type="checkbox" name="usa_indirizzo" id="usa_indirizzo" class="material" /> Usa come dato di contatto</p>
              <p><input type="tel" name="telefono" id="reg_telefono" placeholder="Telefono" class="material" required/><br>
                <input type="checkbox" name="usa_telefono" id="usa_telefono" class="material" /> Usa come dato di contatto</p>
              <p><input type="email" name="email" id="reg_email" placeholder="Email" class="material" required/><br>
                <input type="checkbox" name="usa_email" id="usa_email" class="material" /> Usa come dato di contatto</p>
              ...
              <div class="g--half g--last text-divider" id="div_tempo_default">
                <label>Questo campo indica il tempo (in minuti) entro il quale il fornitore dovrà gestire la tua richiesta.
                  Potrai modificare questo valore in qualsiasi momento dopo la registrazione e prima di inviare una richiesta.</label><br>
              ...
              <div id="campi_fornitore" style="display: none;">
                ...
                <p><input type="text" name="piva" id="reg_piva" placeholder="Partita Iva" class="material" disabled /></p>
                  <span id="piva_non_valida">* Partita IVA non valida.</span>
                <p><select name="attivita" id="reg_attivita" class="material" disabled>
                ...
                <p><textarea type="text" name="descrizione" id="reg_descrizione" placeholder="Descrizione Attività" class="material" disabled></textarea></p>
                <p><select name="modalita" id="reg_pagamento" class="material" disabled>
                  <option value="seleziona" selected disabled>Modalità di pagamento:</option>
                  <option value="paypal">Paypal</option>
                  <option value="bonifico">Bonifico bancario</option>
                </select></p>
                <p><input type="number" name="limite_spesa" id="reg_limite_spesa" placeholder="Limite Spesa Mensile" class="material" disabled /></p>
              </div>
              </div>
		...
		public function get_myAccount_form(){
          ...
          <p>Tempo di attesa della richiesta</p>
	...
	public function esegui_registrazione(){
		...
		if((isset($_POST['register'])) AND (isset($_POST['fornitore'])) AND ($_POST['fornitore']==true)) {
		    ...
		    if(isset($_POST['register']) AND
  			...
  			isset($_POST['attivita']) AND
  			isset($_POST['descrizione']) AND
  			isset($_POST['modalita']) AND
  			isset($_POST['limite_spesa']) AND
  			isset($_POST['email_pushbullet'])){
    			...
    			$this->reg_attivita = trim($_POST['attivita']);
    			$this->reg_descr_attivita = trim($_POST['descrizione']);
    			$this->reg_pagamento = trim($_POST['modalita']);
    			$this->reg_limite_spesa = trim($_POST['limite_spesa']);
	  ...
	  $today = date("Y/m/d", time());
		...
		$query = "
					...
					SET
						...
						tipo_attivita='".mysql_real_escape_string($this->reg_attivita)."',
						descrizione='".mysql_real_escape_string($this->reg_descr_attivita)."',
						modalita_pagamento='".mysql_real_escape_string($this->reg_pagamento)."',
						limite_spesa='".mysql_real_escape_string($this->reg_limite_spesa)."',
						contatti_mensili=0,
          ...
          while($row = mysql_fetch_array($result)) {
            ...
            $html .= '
                <tr>
                    ...
                    <td data-th="Modalità pagamento">'.$row['modalita_pagamento'].'</td>
                    <td data-th="Spesa massima">'.$row['limite_spesa'].'</td>
                    <td data-th="Elimina"><i class="icon icon-close" id="delete_employee" onclick="delete_employee('.$row['id'].')"></i></td>
                </tr>';
          }
    		}
